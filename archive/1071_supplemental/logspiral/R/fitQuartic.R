...
dev.set( dev.list()[1])
        ...
        title("Please mouse (left) click your best guess QQ axis location", 
...
MASS::eqscplot(x, y, pch = 1)
points(start.u, start.v, pch=3, cex=2.5, col=4)
title("Outline with specified initial QQ axis location", 
            cex = 0.9, col.main = 2, adj = 0)
...
#
    lspiral.lhs <- function(x, y, u, v) sqrt((x - u)^2 + (y - 
        v)^2)
    lspiral.rhs <- function(x, y, u, v, a1, k1, q, c, qq) {
        theta <- cumulativeAngle(x, y, u, v)
        a1 * exp(k1 * theta + q * theta^2 + c * theta^3 + qq * 
            theta^4)
    }
    u <- start.u
    v <- start.v
    ...
    spiral.fit <- nls(~lspiral.lhs(x, y, u, v) - lspiral.rhs(x, 
        y, u, v, a1, k1, q, c, qq), trace = F, control = list(warnOnly = TRUE, 
        tol = tolerance.step, maxiter = 400), start = list(u = u, 
        v = v, a1 = a1.start, k1 = k1.start, q = q.start, c = c.start, 
        qq = qq.start))
    ...
    q <- as.numeric(spiral.fit.coef[5])
    c <- as.numeric(spiral.fit.coef[6])
    qq <- as.numeric(spiral.fit.coef[7])
    ...
    rpp1 <- a1 * exp(k1 * thetap1 + q * thetap1^2 + c * thetap1^3 + 
        qq * thetap1^4)
    deviations <- rp1 - rpp1
    ...
    r.fit <- a1 * exp(k1 * seq.theta + q * seq.theta^2 + c * 
        seq.theta^3 + qq * seq.theta^4)
    rfit.a <- a1 * exp(k1 * thetap1 + q * thetap1^2 + c * thetap1^3 + 
        qq * thetap1^4)
    ...
    if (angle.deg) 
        k1 <- (180/pi) * atan(1/k1)
    parameters1 <- round(c(u1, v1, a1, k1, q, c, qq), 4)
    ...
    names(parameters1) <- c("x.axis", "y.axis", "a1", "k1", "quad", 
        "cubic", "quartic")
