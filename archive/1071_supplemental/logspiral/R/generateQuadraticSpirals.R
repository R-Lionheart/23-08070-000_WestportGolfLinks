...
function (nstart = 200, change = 100, k1 = 40, k2 = 50, q1 = 0.03, 
    ...
    plot.it = TRUE, show.change = TRUE) 
{
    ...
    cat("xc", xc, "\n")
    q2 <- (k1 - k2)/(2 * xc) + q1
    log.a2 <- log(a1) + k1 * xc + q1 * xc^2 - k2 * xc - q2 * 
        xc^2
    a2 <- exp(log.a2)
    cat("Parameters: a1", round(a1, 4), " k1", round(k1, 4), 
        " q1", round(q1, 4), " a2", round(a2, 4), " k2", round(k2, 
            4), " q2", round(q2, 4), "\n")
    xr <- ifelse(theta <= xc, log(a1) + k1 * theta + q1 * (theta^2), 
        log(a2) + k2 * theta + q2 * (theta^2))
    xr <- exp(xr)
    x <- xr * cos(theta)
    y <- xr * sin(theta)
    axis.locate <- c(0, 0)
    ...
    cat("  Change at coordinate pair : ", change, "\n")
