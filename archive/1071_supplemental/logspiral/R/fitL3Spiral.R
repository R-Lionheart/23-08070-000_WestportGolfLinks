fitL3Spiral <-
function (x, y, m = c(NULL, NULL), start.u = NULL, start.v = NULL, 
...
#
    ...
    a2 <- a1 * exp((k1 - k2) * zz1)
    a3 <- a2 * exp((k2 - k3) * zz2)
    theta.change <- c(zz1, zz2)
    rpp1 <- ifelse(thetap1 < zz1, a1 * exp(k1 * thetap1), ifelse(thetap1 < 
        zz2, a2 * exp(k2 * thetap1), a3 * exp(k3 * thetap1)))
    deviations <- rp1 - rpp1
    ...
    v.deviations <- ifelse(thetap1 < zz1, deviations * sin(atan(1/k1)), 
        ifelse(thetap1 < zz2, deviations * sin(atan(1/k2)), deviations * 
            sin(atan(1/k3))))
    ...
    r.fit <- ifelse(seq.theta < zz1, a1 * exp(k1 * seq.theta), 
        ifelse(seq.theta < zz2, a2 * exp(k2 * seq.theta), a3 * 
            exp(k3 * seq.theta)))
...
k3 <- (180/pi)*atan(1/k3)
    ...
    parameters1 <- round(c(u1, v1, a1, a2, a3, k1, k2, k3), 4)
