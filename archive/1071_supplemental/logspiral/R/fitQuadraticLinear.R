...
if( is.null(m) ) stop( "Must supply the coordinate location of the transition (m)")
    ...
    lspiral.rhs <- function(x, y, u, v, a1, k1, k2, m = m1) {
        ...
        xc <- theta[m]
        a1 * exp(ifelse(theta <= xc, k1 * theta + ((k2 - k1)/(2 * 
            xc)) * theta^2, (k1 - k2) * xc/2 + k2 * theta))
    }
if (is.null(axis.start)) {
if( length(dev.list()) <1 ) dev.new()
dev.set( dev.list()[1])
        MASS::eqscplot(x, y, pch = 1)
        title("Please mouse (left) click your best guess QL axis location", 
...
title("Outline with specified initial QL axis location", 
            cex = 0.9, col.main = 2, adj = 0)
...
#
    ...
    a2 <- exp(log.a2)
    rpp1 <- ifelse(thetap1 < zz, log(a1) + k1 * thetap1 + q * 
        (thetap1^2), log(a2) + k2 * thetap1)
    rpp1 <- exp(rpp1)
    deviations <- rp1 - rpp1
    ...
    r.fit <- ifelse(seq.theta < zz, log(a1) + k1 * seq.theta + 
        q * (seq.theta^2), log(a2) + k2 * seq.theta)
    r.fit <- exp(r.fit)
    rfit.a <- ifelse(thetap1 < zz, log(a1) + k1 * thetap1 + (q * 
        thetap1^2), log(a2) + k2 * thetap1)
    rfit.a <- exp(rfit.a)
...
k2 <- (180/pi)*atan(1/k2)
    ...
    parameters1 <- round(c(u1, v1, a1, k1, q, a2, k2), 4)
...
if (plot.diagnostics) {
# includes a check on extreme deviations around the change location
if( length(dev.list()) <1 ) dev.new()
