fitBentCable <-
function (x, y, m = NULL, axis.start = c(NULL, NULL), quadratic = TRUE,
 start.values=c( a=2, k1=36, k2=46,g=0.3,kappa= 2.0), angle.deg=TRUE,
...
if (quadratic) 
{  
   lspiral.rhs.q <- function(x, y, u, v, a1, k1, kdiff, g, m = m1) {
      ...
      quad.linear <- ((theta.d + g)^2/(4 * g)) * 
        ...
        a1 * exp(k1 * theta + kdiff * quad.linear)
    }
}
else
{
    lspiral.rhs.g <- function(x, y, u, v, a1, k1, kdiff, g,kappa, m = m1) {
        theta <- logspiral::cumulativeAngle(x, y, u, v)
        xc <- theta[m]
        tau <- xc
        theta.d <- theta - xc
...
#
if (is.null(axis.start)| length(axis.start) < 2) stop("Must explicitly input initial axis location as two values ",call.=FALSE)
    u <- axis.start[1]
    v <- axis.start[2]
...
title("Outline with specified initial BC axis location", 
            cex = 0.9, col.main = 2, adj = 0)
#
if ( length(start.values) !=5) stop ("Must input FIVE starting parameter values see help(fitBentCable)")
start.values <- c(start.values["a"], start.values["k1"], start.values["k2"], start.values["g"], start.values["kappa"])
    a1 <- as.numeric( start.values["a"])
    ...
    kdiff <- k2-k1
    g <- as.numeric(start.values["g"])
kappa <- ifelse( quadratic, 2,as.numeric(start.values["kappa"])  )
# 
if(angle.deg) {
#cat("\n")
cat( "Degrees in start, so radian slope start values are ","\n")
cat( "   k1   k2 :    ", c(k1,k2),"\n")
}  else
{
cat("\n")
cat( "Radian in start, so as degrees have start values :" ,"\n")
cat( " k1 k2   : ", c((180/pi)*atan(1/c(k1,k2) ) ),"\n")
}
...
if (quadratic)
{
    spiral.fit <- nls(~lspiral.lhs(x, y, u, v) - 
        lspiral.rhs.q(x, y, u, v, a1, k1, kdiff, g), 
         trace = trace.check, control = list(tol = tolerance.step, 
        maxiter = 400), 
      start = list(u = u, v = v, a1 = a1, k1 = k1, kdiff = kdiff, g = g))
    ...
    kdiff <- as.numeric(spiral.fit.coef[5])
    g <- as.numeric(spiral.fit.coef[6])

}
else
{
    spiral.fit <- nls(~lspiral.lhs(x, y, u, v) - 
            lspiral.rhs.g(x, y, u, v, a1, k1, kdiff, g,kappa), 
         trace = trace.check, control = list(tol = tolerance.step, 
        maxiter = 400), start = list(u = u, v = v, 
        a1 = a1, k1 = k1, kdiff = kdiff, g = g, kappa = kappa))
    spiral.fit.coef <- coef(spiral.fit)
    u1 <- as.numeric(spiral.fit.coef[1])
    v1 <- as.numeric(spiral.fit.coef[2])
    a1 <- as.numeric(spiral.fit.coef[3])
    ...
    kdiff <- as.numeric(spiral.fit.coef[5])
    g <- as.numeric(spiral.fit.coef[6])
    kappa <- as.numeric(spiral.fit.coef[7])
}
    k2 <- k1 + kdiff
...
# tau is user fixed change location, and NOT estimated 
# 
...
#
   rpp1 <- a1 * exp(k1 * thetap1 + kdiff * zcode)
    deviations <- rp1 - rpp1
...
#
    rfit <- NULL
    rfit <- a1 * exp(k1 * seq.theta + kdiff * zzcode)
    rfit.a <- a1 * exp(k1 * thetap1 + kdiff * zcode)
...
k2 <- (180/pi)*atan(1/k2)
    ...
    parameters1 <- round(c(u1, v1, a1, k1, a2, k2, kdiff, g,kappa, tau), 4)
...
if (plot.deviations) {
if( length(dev.list()) <1 ) dev.new()
if( length( dev.list()) < 3) warning("Useful for  THREE+ graphic windows open see doSetupGraphics")
dev.set( dev.list()[3])
...
{
#
# check for extreme deviations around the change location (+/- 4 coords and 95% quantile)
#
...
if (plot.diagnostics) {
# includes a check on extreme deviations at change location
if( length(dev.list()) <1 ) dev.new()
