...
title("Outline with specified initial C axis location", 
            cex = 0.9, col.main = 2, adj = 0)
...
title("Outline with specified initial C axis location", 
    ...
    u <- start.u
    v <- start.v
    k1.start <- 1/(tan(angle.start * (pi/180)))
    q.start <- quadratic.start
    c.start <- cubic.start
    spiral.fit <- nls(~lspiral.lhs(x, y, u, v) - lspiral.rhs(x, 
        y, u, v, a1, k1, q, c), trace = F, control = list(warnOnly = TRUE, 
        tol = tolerance.step, maxiter = 400), start = list(u = u, 
        v = v, a1 = dist.zero, k1 = k1.start, q = q.start, c = c.start))
    ...
    if (angle.deg) 
        k1 <- (180/pi) * atan(1/k1)
    parameters1 <- round(c(u1, v1, a1, k1, q, c), 4)
