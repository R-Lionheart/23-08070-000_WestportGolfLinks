...
2.  Where did the 10ft/year R0 northern migration come from? **Professional estimate?**
...
distance_wide <- pivot_wider(distance_to_baseline,names_from = year, values_from = distance_f)
```
...
```{r}
distance.plot <- ggplot(distance_to_baseline, aes(group=position, y=distance_f, x=year, color=position)) +
...
ROC.plot <- ggplot(data=ROC, aes(x=year, y=rate, group=position, color = position)) +
  ...
  scale_color_manual(values = c('#5C4E2C', '#395C51', "#036C88", "#36A886","#04A1FF", "#DBA827"), 
                     name = "Geographic location") +
  xlab("Year") + 
  ylab("Rate of Change (feet/year)") +
  ggtitle("Rate of Change (feet/year)")
ROC.plot
```
...
prediction <- distance_to_baseline %>%
  ...
  mutate(time_diff = max(year) - min(year),
         distance_diff = distance_f[year == 2022] - distance_f[year == 2006],
